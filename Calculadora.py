...
class Calculadora(object):
    ...
    def __init__(self):
        ...
        ttk.Button(mainframe, text='CE', command=partial(self.borrarPanel, 'CE')).grid(column=3, row=8, sticky=W)
    ...
    def ponerNUMERO(self, numero):
        if self.__operadorAux!=None:
            ...
            if self.__panel.get().count('/')==1:
                ...
                valorN=separador[0]
                valorD=separador[1]
                self.__primerOperando=Fraccion(int(valorN),int(valorD))
    ...
    def ponerOPERADOR(self, op):
        ...
        if op=='=':
            if self.__panel.get().count('/')==1:
                ...
                valorN=separador[0]
                valorD=separador[1]
                self.__segundoOperando=Fraccion(int(valorN),int(valorD))
            else:
